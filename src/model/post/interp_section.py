...
from model.post.is_fcn.mesh import mesh
from model.post.is_fcn.remesh_step import remesh_step
...
def interp_section(ifrd_data,nsets,ndisk=10):
    ...
    for i in range(64):
        if i == 0:
            ...
            prefix = "P"+str(i+1)+"_"
        id_nodes = [ nsets[prefix+"Yarn_"+str(i+1)+"_"+str(k)]
                    for k in range(1,4) ]
        id_nodes = np.concatenate(id_nodes)
