...
"""
    ...
    if yukawa_cutoff is None:
        yukawa_cutoff = 2*(params["r_hebra"] + params["r_hebra_offset"])
    ...
    for i in merge:
        ...
        all_bonds += pairs
    # ==================================================================
    params["all_bonds"] = np.array(all_bonds)
    
    for i in range(len(all_bonds)):
        # init point of traj all_bonds[i][0]
        # end point of traj all_bonds[i][1]
        index_init = clens[all_bonds[i][0]]
        index_end  = clens[all_bonds[i][1]-1]+1
        lines = "create_bonds  single/bond 1 {} {}\n".format(index_init,index_end)
        lammps_file.write(lines)
...
timestep  0.005
dump		2 all custom 100 dump.0.xyz id type xu yu zu mol
