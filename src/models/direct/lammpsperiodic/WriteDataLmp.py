...
def WriteDataLmp(params,name_file):
    ...
    # Limits of box
    # =======================================================
    xall = np.concatenate([trajs[i][:,0] for i in range(len(trajs))])
    yall = np.concatenate([trajs[i][:,1] for i in range(len(trajs))])
    x_lim = [ np.min(xall) , np.max(xall) ]
    y_lim = [ np.min(yall) , np.max(yall) ]

    x_lim = [ x_lim[0]*factor, x_lim[1]*factor]
    y_lim = [ y_lim[0]*factor, y_lim[1]*factor]

    z_lim = [0, h*params["len_periodic"]] # h/8
